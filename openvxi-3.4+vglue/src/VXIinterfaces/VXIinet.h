...
/**
 * \defgroup VXIinet Internet Interface
 *
 ...
 * 
 * Keys identifying properties in VXIMap for Prefetch( ) and Open( ):
 * VXIinet functions take a VXIMap argument which contains a set of
...
/**
 * @name VXIinet caching property values
 * Set of defined caching property values that are used to control
...
/**
 * @name INET_SUBMIT_METHOD supported property values
 * Full HTTP 1.1 support could be added to this interface. The
 ...
 * 
 * Default values for properties in the VXIMap argument for
 * Prefetch() and Open().  If the properties are not set in 
 * the call to these function the given default value will 
 * be assumed.
 */
/*@{*/
 ...
 * Data source values.
 *
 * An enum in the Inet streamInfo map to indicate the data source: 
 * from a local file, from the fetch cache, from the fetch cache but had to 
 * revalidate with the web server, or from a full Internet fetch.
 */
typedef enum VXIinetDataSource {
  /** Open for reading from a local file */
  INET_DATA_SOURCE_FILE       = 0x0,

  /** Open for reading from a full Internet fetch */
  INET_DATA_SOURCE_HTTP       = 0x1,

  /** Open for reading from a FTP fetch */
  INET_DATA_SOURCE_FTP        = 0x2,

  /** Open for reading from the cache */
  INET_DATA_SOURCE_CACHE      = 0x3,

  /** Open for reading from the cache but had to revalidate */
  INET_DATA_SOURCE_VALIDATED  = 0x4
...
typedef enum VXIinetOpenMode {
  ...
  /** Open for reading as with INET_MODE_READ, but
   *  returns a local file path instead of an open stream.
   */
  INET_MODE_FILE    = 0x2
} VXIinetOpenMode;
 ...
 * @name Open flags
 *
 * Flags for Open( ), may be combined through bitwise or.
 * The Open() call takes a bitwise or of open flags which control
 ...
 * Keys identifying properties in VXIMap used to return stream
 * information for Open( ).
 * The VXIinet implementation determines information about the
...
#define INET_INFO_SIZE_BYTES     L"inet.info.sizeBytes"
  ...
  /** Local file path of cached content */
#define INET_INFO_LOCAL_FILE_PATH     L"inet.info.localFilePath"

  /** Pointer to parse tree of VXML document */
#define INET_INFO_PARSE_TREE     L"inet.info.parseTree"
...
#define INET_INFO_EXPIRES_HINT  L"inet.info.expiresHint"
  ...
  /**
   * an enum of type VXIinetDataSource (see above) in the Inet streamInfo 
   * map to indicate the data source: 
   * from a local file, from the fetch cache, from the fetch cache but had to 
   * revalidate with the web server, or from a full Internet fetch.
   * 
   */
...
typedef struct VXIinetInterface {
   ...
   * Close a stream that was previously opened. Closing a NULL or
   ...
   *                           outputting the error. See the top of VXIlog.h
   *                           for moduleName allocation rules.
   * @param name          [IN] Name of the data to canonicalize, see above for
   *                           supported types
   * @param properties    [IN] Properties to control the canonicalization,
   *                        may be NULL. Of particular note are:
   *                        INET_URL_BASE, base URL for resolving relative URLs
   *                        INET_URL_QUERY_ARGS, map containing key/value
   *                        pairs for HTTP queries, where the name of each
   *                        key is the query argument name, and the value of
   *                        each key is a VXIValue subclass that provides the
   *                        value for that argument
   * @param absoluteName  [OUT] The absolute URI after the canonicalization,
   *                            may be NULL if an error occured.
   *
   * @return VXIinet_RESULT_SUCCESS on success
   */   				
  VXIinetResult (*GetAbsoluteName)(struct VXIinetInterface  *pThis,
                                   const VXIchar            *moduleName,
                                   const VXIchar            *name,
                                   const VXIMap             *properties,
